...
public class UserController {
     ...
     */
    @PutMapping("/")
    public ResponseEntity<User> updateUserInfo(@RequestParam(value = "password", required = false) String password, @RequestParam("username") String username) {
        User user = userService.getUserByUsername(username);
        if (user == null) {
            throw new NoUserException();
        } else {
     ...
     * @return 原用户信息
     *
     * <p>删除用户，采用delete方法传递信息。若未找到用户抛出错误返回404码。若找到则删除用户并清空有关的文件和授权信息并返回其信息。</p>
     */
